...
def import_gmd_bytes_to_collection(context, file_label: str, file_bytes: bytes, prefer_skinned=True, strict=True, gmd_folder: str = None):
    """Module-level helper to import GMD bytes into a new collection in the given context.

    Returns the created collection on success or raises an Exception on failure.
    """
    # Create a basic error reporter
    debug_categories = set(["ALL"]) if strict else set()
    from ..error_reporter import BlenderErrorReporter
    from ...gmdlib.errors.error_reporter import StrictErrorReporter, LenientErrorReporter
    base_error_reporter = StrictErrorReporter(debug_categories) if strict else LenientErrorReporter(debug_categories)
    error = BlenderErrorReporter(lambda *args, **kwargs: None, base_error_reporter)

    # parse structures
    gmd_version, gmd_header, gmd_contents = read_gmd_structures(file_bytes, error)

    # map version to engine enum
    engine_from_version = {
    ...
    # default GMDSceneCreatorConfig values (conservative defaults)
    ...
    # default to collection_texture naming
    config = GMDSceneCreatorConfig(
        game=engine_enum,
        import_materials=True,
        material_naming_convention=material_naming_from_enum["COLLECTION_TEXTURE"],
        fuse_vertices=True,
    ...
    if prefer_skinned:
        try:
            gmd_scene = read_abstract_scene_from_filedata_object(gmd_version, FileImportMode.SKINNED,
                                                                 ...
                                                                 gmd_contents, error)
            scene_creator = GMDSkinnedSceneCreator(filepath_for_creator, gmd_scene, config, error)
...
class BaseImportGMD:
    ...
    def import_from_bytes(self, context, file_label: str, file_bytes: bytes, prefer_skinned=True, error=None):
        """Import a GMD from raw bytes. Returns the created collection or raises on fatal error.

        This centralizes the import flow so callers (including the PAR browser) can import on-demand
        without duplicating parsing/scene-creation code.
        """
        if error is None:
        ...
        if prefer_skinned:
            try:
                ...
                scene_creator = GMDSkinnedSceneCreator(file_label, gmd_scene, gmd_config, error)
                scene_creator.validate_scene()
                gmd_collection = scene_creator.make_collection(context)
                gmd_armature = scene_creator.make_bone_hierarchy(context, gmd_collection)
                scene_creator.make_objects(context, gmd_collection, gmd_armature)
        ...
        try:
            gmd_scene = read_abstract_scene_from_filedata_object(gmd_version, FileImportMode.UNSKINNED,
                                                                 ...
                                                                 gmd_contents, error)
            gmd_config = self.create_gmd_config(gmd_version, error)
            scene_creator = GMDUnskinnedSceneCreator(file_label, gmd_scene, gmd_config, error)
            scene_creator.validate_scene()
            gmd_collection = scene_creator.make_collection(context)
            scene_creator.make_objects(context, gmd_collection)
            return gmd_collection
...
def menu_func_import_skinned(self, context):
    # menu entry removed: label served no purpose
    pass
