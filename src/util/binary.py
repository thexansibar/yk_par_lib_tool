...
class BinaryReader:
    ...
    def push(self):
        self.__pos_stack.append(self.__idx)

    def pop(self):
        self.__idx = self.__pos_stack.pop()
    ...
    def read_bytes(self, size=1):
        temp_idx = self.__idx
        self.__idx += size
        # return exactly `size` bytes (avoid off-by-one)
        return self.__buf[temp_idx: temp_idx + size]

    def read_int64(self, count=1):
        if count > 1:
            return self.__read_type("q", count)
        return self.__read_type("q", count)[0]
    ...
    def write_int64(self, value, count=-1, is_iterable=False):
        return self.__write_type("q", value, count, is_iterable)

    def write_uint64(self, value, count=-1, is_iterable=False):
        return self.__write_type("Q", value, count, is_iterable)
