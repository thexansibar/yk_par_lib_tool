...
class ParentStack:
    ...
    def handle_node(self, stack_op: NodeStackOp, to_push: GMDNode):
        if stack_op in [NodeStackOp.PopPush, NodeStackOp.Pop]:
            if not self.stack:
                self.error.recoverable("Trying to pop() out the top of the GMD."
                ...
                self.stack.pop()
        # TODO need to figoure out how to hande PopPush BlendShapes
        if stack_op in [NodeStackOp.PopPush, NodeStackOp.Push] and not isinstance(to_push, GMDBlendShape):
...
class GMDAbstractor_Common(abc.ABC, Generic[TFileData]):
    ...
    def build_node_hierarchy_from_structs(self,
        ...
        # def create_gmd_node(node_struct: NodeStruct, parent) -> GMDNode:
